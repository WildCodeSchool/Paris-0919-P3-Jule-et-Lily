...
router.route(['/image-cover/:id', '/image-cover/:id/:collectionId'])
...
`SELECT * FROM image JOIN collection ON image_id = collection_cover_image_id WHERE collection_id= ${req.params.id}`,
  (err, results) => {
    if (err) {
      res
        .status(500)
        .send("Erreur lors de la récupération de l'image de couverture de la collection");
        console.log("erreur  recup image", err)
    } else {
      res.json(results);
    }
...
connection.query(`UPDATE collection SET collection_cover_image_id=${ImageId} WHERE collection_id=${collectionId}`, (err, results) => {
  if (err) {
    console.log('erreur back', err);
    res.status(500).send("Erreur lors de la modification de l'image de couverture de la collection");
  } else {
    console.log(results)
    res.sendStatus(200);
  }

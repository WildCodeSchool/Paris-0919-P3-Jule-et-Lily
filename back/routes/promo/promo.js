...
router.route([`${codePromoRoute}:id`, codePromoRoute])
  .get(function (req, res) {
    ...
    connection.query(`SELECT * FROM promo WHERE promo_id=?`, [idCodePromo], (err, results) => {
      if (err) {
  ...
  .put(function (req, res) {
    const idCodePromo = req.params.id;
    const formData = req.body;
    connection.query('UPDATE promo SET ? WHERE promo_id=?', [formData, idCodePromo], err => {
      if (err) {
  ...
  .delete(function (req, res) {
    const idCodePromo = req.params.id;
    connection.query('DELETE FROM promo WHERE promo_id=?', [idCodePromo], err => {
      if (err) {

...
router.route(['/stock/:id','/stock/:stock_quantity/and/:stock_min'])
  ...
  .put(function (req, res) { // modifier le stock
...
router.route(['/all'])
  .get(function (req, res) { // récup tout les produit
...
router.route(['/:id', '/'])
  ...
  .post(function (req, res) {
    ...
    connection.query('SET FOREIGN_KEY_CHECKS=0', formData, (err, results) => { 
      if (err) {
  ...
  .delete(function(req, res) {   //supprimer un produit et ses images associées
